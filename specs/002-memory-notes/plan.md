...
**Verified against** the tree at `969ba0d`. 001 already shipped the
trees, the parser, `/memory`, and FTS that skips sessions. 002 closes
the hole where status is a lie.
...
- `check_ml.ml:315-321` asserts the memory *visual*, not that FTS
  found the fixture or excluded a session
- `find memory -type f` is still a README and four `.gitkeep`s. The
  first real note is `003-harness-and-hygiene#T070`, not this plan

## Order

```
T010 spec acceptance
  └─ T020 status is optional, not Active-by-default
       └─ T030 is_reviewed, then load + FTS
            ├─ T040 check pins the set
            └─ T050 /memory shows status
```

`src/` and `_generated/` stay one commit.
